          ...
          >
            ...
            {code ? (
              <div className="text-center">
                ...
                <div className="bg-gold-50 rounded-2xl py-5 px-4 mb-4">
                  <p className="text-4xl font-mono font-bold text-gold-600 tracking-widest select-all">{code}</p>
                </div>
                <button onClick={copyCode} className="flex items-center gap-1.5 mx-auto text-sm text-gold-600 hover:text-gold-700 transition-colors cursor-pointer">
                  <Copy size={14} /> 复制加入码
                </button>
              </div>
            ) : (
              <>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:border-gold-400 focus:ring-1 focus:ring-gold-200 transition-colors bg-white/80 mb-3"
                  autoFocus
                />
                {error && <p className="text-red-500 text-xs mb-3">{error}</p>}
                <button onClick={handleCreate} disabled={loading || !name.trim()}
                  className="w-full py-2.5 rounded-xl bg-gold-500 text-white text-sm font-medium hover:bg-gold-600 transition-colors disabled:opacity-50 cursor-pointer">
                  {loading ? "创建中..." : "创建"}
                </button>
              </>
            )}
        ...
        >
          ...
          >
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:border-gold-400 focus:ring-1 focus:ring-gold-200 transition-colors bg-white/80 mb-3 font-mono"
              autoFocus
            />
            {error && <p className="text-red-500 text-xs mb-3">{error}</p>}
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setJoinOpen(true)}
              className="flex items-center gap-1.5 px-4 py-2 rounded-full border border-gold-500 text-gold-600 text-sm font-medium hover:bg-gold-50 transition-colors cursor-pointer">
              <UserPlus size={16} />
              加入班级
            </button>
            {profile?.role === "teacher" && (
              <button onClick={() => setCreateOpen(true)}
                className="flex items-center gap-1.5 px-5 py-2 rounded-full bg-gold-500 text-white text-sm font-medium hover:bg-gold-600 transition-colors cursor-pointer">
                <Plus size={16} />
                创建班级
              </button>
            )}
            ...
            {classes.map((cls, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-gold-50 flex items-center justify-center mb-4">
                  <Users size={20} className="text-gold-600" strokeWidth={1.5} />
                </div>
                ...
                <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-gold-50 text-gold-600 font-medium">
